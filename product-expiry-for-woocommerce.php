...
class WOO_Product_Expiry {
    ...
    function save_settings(){
        // Verify the nonce
        if (isset($_REQUEST['woope_save_admin_settings_nonce']) && wp_verify_nonce($_REQUEST['woope_save_admin_settings_nonce'], 'woope_save_admin_settings_nonce') && current_user_can('manage_options')) {
            ...
            if (update_option( 'woope_admin_settings', $settings )) {
                ...
                echo __( 'Unable to update, or you may not changed any setting.', 'product-expiry-for-woocommerce' );
            }
    ...
    */
    ...
    function save_variation_fields($post_id){
        ...
        if (isset($_POST['_woope_exp_date'][ $post_id ]) && $_POST['_woope_exp_date'][ $post_id ] == '') {
            wp_clear_scheduled_hook( 'woo_expiry_schedule_action', array($post_id) );
        }
